...
export function buildQuickActions(workspace: any): QuickAction[] {
  ...
  if (workspace?.prReview) {
    ...
    actions.push({
      ...
      prompt: "Review the code in this workspace and suggest improvements beyond what was found in the PR review.",
      icon: Sparkles,
  ...
  if (workspace?.task || workspace?.taskId) {
    ...
    const taskTitle = task?.title || workspace.name;
    const taskDesc = task?.description || workspace.description || "";
